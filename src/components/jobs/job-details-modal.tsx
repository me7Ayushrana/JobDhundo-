      ...
      {isOpen && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 md:p-6 overflow-hidden">
          ...
          <motion.div
            ...
            className="relative bg-white dark:bg-stone-900 w-full max-w-5xl h-[85vh] rounded-3xl border border-black/5 dark:border-white/5 shadow-2xl flex flex-col z-10 overflow-hidden"
          >
            {/* Header Area */}
            <div className="p-6 md:p-8 border-b border-stone-100 dark:border-stone-800 flex justify-between items-start">
              <div className="flex gap-4 items-start pr-8">
                {job.companyLogo ? (
                  ...
                  <div className="w-14 h-14 rounded-2xl bg-stone-100 dark:bg-stone-800 flex items-center justify-center font-black text-stone-600 dark:text-stone-300 text-lg border border-stone-200 dark:border-stone-700 mt-1">
                    {job.company.charAt(0)}
                  </div>
                )}
                <div>
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <span className="text-sm font-semibold text-stone-500 dark:text-stone-400">{job.company}</span>
                    <Badge variant="outline" className="text-[10px] uppercase font-bold py-0 bg-stone-50 dark:bg-stone-800 text-stone-600 dark:text-stone-400 border-none">
                      {job.sourceAttribution}
                    </Badge>
                  </div>
                  <h2 className="text-xl md:text-2xl font-black text-stone-900 dark:text-stone-50 leading-tight">
                    {job.title}
                  </h2>
                </div>
              </div>
              ...
              <button
                onClick={onClose}
                className="p-2.5 rounded-full hover:bg-stone-100 dark:hover:bg-stone-800 text-stone-400 hover:text-stone-600 dark:hover:text-stone-200 transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-6">
                <div>
                  <h3 className="text-sm font-black text-stone-900 dark:text-stone-100 uppercase tracking-widest mb-3 flex items-center gap-2">
                    <Sparkles className="w-4 h-4 text-primary" /> Job Description
                  </h3>
                  <div className="text-sm text-stone-600 dark:text-stone-300 leading-relaxed space-y-3 font-medium whitespace-pre-line">
                    {job.description}
                  </div>
                </div>
                ...
                {job.requirements && job.requirements.length > 0 && (
                  <div>
                    <h3 className="text-sm font-black text-stone-900 dark:text-stone-100 uppercase tracking-widest mb-3">
                      Key Qualifications
                    </h3>
              ...
              <div className="space-y-6">
                {/* Quick Info Box */}
                <div className="p-5 rounded-2xl bg-stone-50 dark:bg-stone-800/40 border border-stone-100 dark:border-stone-800 space-y-4">
                  <h4 className="text-xs font-black text-stone-400 uppercase tracking-widest">Opportunity Details</h4>
                  
                  <div className="space-y-3 text-sm">
                    <div className="flex items-center gap-3 text-stone-700 dark:text-stone-300 font-semibold">
                      <MapPin className="w-4.5 h-4.5 text-stone-400" />
                      <span>{job.location}</span>
                    </div>

                    <div className="flex items-center gap-3 text-stone-700 dark:text-stone-300 font-semibold capitalize">
                ...
                <div className="p-5 rounded-2xl border border-primary/10 bg-primary/5 flex items-center justify-between">
                  ...
                  <div className="w-14 h-14 rounded-full border-4 border-primary/20 flex items-center justify-center font-black text-lg text-primary bg-white dark:bg-stone-950">
                    {score}
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {job.skills.map((skill, sIdx) => {
                      ...
                      return (
                        <div key={sIdx} className="flex items-center justify-between text-sm py-1.5 px-3 rounded-xl bg-stone-50 dark:bg-stone-800/40 border border-stone-100 dark:border-stone-800">
                          ...
                          {hasSkill ? (
                            ...
                            <Badge variant="outline" className="text-stone-400 dark:text-stone-600 text-[10px] py-0 px-2 rounded-full font-bold">
                              Missing
                            </Badge>
                          )}
                        </div>
            ...
            <div className="p-6 border-t border-stone-100 dark:border-stone-800 bg-stone-50 dark:bg-stone-900/60 flex items-center justify-end gap-3">
              <Button
                variant="outline"
                onClick={() => onSave?.(job)}
